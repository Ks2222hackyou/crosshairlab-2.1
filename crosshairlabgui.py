...
with gui.window(label='cross', width=400,height=400,no_title_bar=True,no_resize=True, no_move=True, show=True):
    with gui.tab_bar(label='cross'): 
        with gui.tab(label='default'):
            ...
            gui.add_button(label='generate',callback=thread)
        ...
        with gui.tab(label='position'):
            gui.add_slider_int(label='posx', min_value=1,max_value=screenx,default_value=screenx/2, tag='posx')
            gui.add_slider_int(label='posy', min_value=1,max_value=screeny,default_value=screeny/2, tag='posy')
            gui.add_button(label='reset',callback=reset)
